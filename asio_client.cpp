...
void do_work(asio::io_context* io_context, std::string hostname, std::string port, size_t tries, uint64_t* times) {
  ...
  for (uint64_t j = 0; j < tries; ++j) {
    ...
    asio::write(s, asio::buffer(request, MSG_SIZE));

    char reply[MSG_SIZE];
    size_t reply_length = 0;
    while (reply_length < MSG_SIZE) {
      reply_length += asio::read(s,asio::buffer(reply, MSG_SIZE - reply_length));
    }
...
int main(int argc, char* argv[]) {
  try {
    if (argc != 5)
    {
      std::cerr << "Usage: blocking_tcp_echo_client <host> <port> <tries> <nrthreads>\n";
      return 1;
    }
